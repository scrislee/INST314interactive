...
+ The `learnrhash` package provides some ability for student progress tracking through these workbooks. At the end of each workbook there is a subsection called *Submit*, which includes a button for you to generate a `hash` code which represents your completed Tutorial. They will submit these hash codes using a Google Forms.  Note that the form requires you to be logged into your University of Maryland Google account (don't we all have at least two Google accounts these days?). 
